...
# List of concepts organized by unit
concepts = [
...
def create_question(unit_number):
    requested_unit = [unit for unit in concepts if unit['unit_number'] == unit_number]
    other_units = [unit for unit in concepts if unit['unit_number'] != unit_number]

    if not requested_unit:
        return "No unit found with the specified unit number."

    requested_unit = requested_unit[0]
    concept_keys = list(requested_unit['concepts'].keys())
    random.shuffle(concept_keys)  # Shuffle to randomize the selection

    question_key = random.choice(concept_keys)  # Select one key to be the question
    correct_answer = requested_unit['concepts'][question_key]

    # Prepare wrong answers
    wrong_answers = []
    while len(wrong_answers) < 5:
        other_unit = random.choice(other_units)
        other_keys = list(other_unit['concepts'].values())
        wrong_answers.extend(other_keys)
        random.shuffle(wrong_answers)
        wrong_answers = wrong_answers[:5]

    if correct_answer in wrong_answers:
        wrong_answers.remove(correct_answer)
    wrong_answers = random.sample(wrong_answers, min(4, len(wrong_answers)))  # Ensure we don't exceed the number of wrong answers available

    all_options = [correct_answer] + wrong_answers
    random.shuffle(all_options)  # Shuffle to mix correct and wrong answers

    print(f"\nWhat is {question_key}?")
    option_labels = 'ABCDE'
    options = {option_labels[i]: option for i, option in enumerate(all_options)}
    for label, option in options.items():
        print(f"{label}) {option}")

    return question_key, options, correct_answer

def run_quiz():
    unit_number = int(input("Enter the unit number for your quiz: "))
    quiz_type = input("Do you want a fixed number of questions (a) or keep going until you get k correct in a row (b)? [a/b]: ").lower()

    if quiz_type == 'a':
        total_questions = int(input("How many questions do you want in the quiz? "))
        score = 0
        for _ in range(total_questions):
            question_key, options, correct_answer = create_question(unit_number)
            user_answer = input("Enter your answer (A, B, C, D, E): ").upper()
        ...
        correct_in_row_needed = int(input("How many correct in a row to finish the quiz? "))
        correct_in_row_count = 0
        total_attempts = 0
        while correct_in_row_count < correct_in_row_needed:
            question_key, options, correct_answer = create_question(unit_number)
            user_answer = input("Enter your answer (A, B, C, D, E): ").upper()
            if user_answer in options and options[user_answer] == correct_answer:
                print("Correct!")
                correct_in_row_count += 1
                total_attempts += 1
            else:
                print(f"Wrong! The correct answer was: {correct_answer}")
                correct_in_row_count = 0  # Reset the counter if the answer is wrong
                total_attempts += 1
        print(f"\nYou completed the quiz with {correct_in_row_needed} correct in a row after {total_attempts} attempts.")
