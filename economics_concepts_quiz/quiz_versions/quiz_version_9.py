...
# with open('concepts_as_lists.py', 'r') as file:
#     exec(file.read())
...
def create_question(unit_number):
    # Filter out the requested unit and others for wrong answers
    ...
    wrong_answers.remove(correct_answer) if correct_answer in wrong_answers else None
    wrong_answers = random.sample(wrong_answers, 4)  # Only need 4 wrong answers
    
    all_options = [correct_answer] + wrong_answers
    random.shuffle(all_options)  # Shuffle to mix correct and wrong answers
    
    # Formatting question and options
...
def run_quiz():
    # User inputs the unit number for the quiz
